...
Biodiversity measurements incorporate many recently developed technologies, such as remote sensing or DNA sequencing, which is particularly useful for small organisms. However, these techniques require numerous processing steps that can influence the final results. In this exercise, we will explore certain parameters that can be adjusted and their impact on the diversity measures.
...
<br>
<br>
In eDNA metabarcoding, we use short DNA sequences called primers to target specific genes in environmental samples. These primers come in pairs:
...
When sequencing, we get DNA reads from both directions (forward and reverse sequences) which help improve accuracy by overlapping and confirming each other and allow to find a more precise taxonomy (e.g. name of the species).
...
Note that DNA metabarcoding is not quantitative per se (i.e. to get species abundance data), as potential biases can be present at each steps.
...
BUT is it reliable for measuring the specific richness ?  
<br>
