---
title: "Metabarcoding intro and DADA2 pipeline"
author: "Alexis Carteron"
date: "`r Sys.time()`"
output: 
  html_document:  
...
Biodiversity measurements incorporate many recently developed technologies, such as remote sensing, AI-powered image recognition but also DNA metabarcoding, which is particularly useful for small organisms. However, these techniques require numerous processing steps that can influence the final results. *In this exercise, we will explore certain parameters of the bio-informatic pipelines that can be adjusted and their impact on the diversity measures.*

<br>

# General introduction on metabarcoding
<br>

Metabarcoding is the use of a polymerase chain reaction (aka PCR) and high‐throughput sequencing to characterize organisms present in a sample. It's called environmental DNA (eDNA) metabarcoding when focusing on the identification of whole communities in a natural sample.  

<br>

Traditional vs eDNA 
<center>
![](Other_materials/elbrecht_slide.png)
</center>
<br>
<br>
NB: In eDNA metabarcoding, we use short DNA sequences called primers to target specific genes in environmental samples. These primers come in pairs:
- Forward primer: It binds to one end of the DNA strand and starts copying in one direction.
- Reverse primer: It binds to the opposite end of the complementary strand and copies in the other direction.
When sequencing, we get DNA reads from both directions (forward and reverse sequences) which help improve accuracy by overlapping and confirming each other and allow to find a more precise taxonomy (e.g. name of the species). (see figure below).

<center>
![](Other_materials/primers.png) 
</center>
<br>
<br>

There are several steps in DNA metabarcoding, see figure below from [Lamb et al., 2019](https://onlinelibrary.wiley.com/doi/abs/10.1111/mec.14920).  
NB: that DNA metabarcoding is not quantitative per se (i.e. to get species abundance data), as potential biases can be present at each steps.

<center>
![](Other_materials/steps_limits.png) 
</center>
<br>
<br>

It is not reliable for abundance data but *is it reliable for measuring the species richness* (i.e. the number of species)? What are the steps and their impacts in bio-informatic pipelines?  
Species richness is a component of biodiversity and used for example to determine biodiversity hotspots (amongst others parameters, see [Sonne & Rahbek, 2024](https://www.pnas.org/doi/10.1073/pnas.2313106121)).  
<br>

# Bio-informatics with DADA2

DADA2 is a bioinformatics pipeline created by [Callahan et al., 2016](https://www.ncbi.nlm.nih.gov/pubmed/27214047). It consists is a series of steps which filter the raw sequences obtained with Illumina sequencing. The final step is to obtain the taxonomy of the sequences that have been filtered in order to study the ecological community.  

DADA2 has two major features which distinguishes it from other pipelines. On one hand, it will proceed to the modeling of the sequencing error  which is supposed to make it possible to distinguish mutant sequences from erroneous sequences. On the other hand, DADA2 does not cluster 97% similar sequences in Operational Taxonomy Units (OTUs). Its **Amplicon Sequence Variants** (ASVs) are not grouped if the sequences are not 100% identical when errors are corrected. See figure below. 


This figure taken from [Hugerth and Andersson, 2017](https://www.ncbi.nlm.nih.gov/pubmed/28928718) illustrates the theoretical difference between OTUs and ASV. Each color represents a clade (e.g. a species). Yellow stars indicate mutations, red stars indicate amplification or sequencing errors. The size of the space between the sequences indicates their clustering.
...
Question: Which approach results in more groups? Why is it problematic?

<br>
<br>

DADA2 was originally constructed for 16S marker gene sequences (Bacteria), but we will use it with ITS marker gene (Fungi) sequences from Illumina MiSEQ 2x300 bp paired-end sequencing. To speed up the execution of each step, we randomly sub-sampled a dataset in order to only keep 1000 sequences per sample.
...
Modify some parameters of the filtering process and check the effects on the number of reads that passed. For example, try to have reads of better quality. See ?filterAndTrim for more details.
...
>#### <span style="color:red">CHALLENGE</span> 
What happen if you want to infer faster by changing the pool argument? See ?dada for more details.
...
>#### <span style="color:red">CHALLENGE</span> 
What happen in terms of number of sequences or ASV richness if you think that a minimum overlap of 12 is too short? See ?mergePairs for more details.
...
>#### <span style="color:red">CHALLENGE</span> 
Here again, the 'method' can be different...
<br>
...
bar_track <- ggplot(lgtrack ,aes(x=ID, y=as.numeric(value), fill=variable)) +
  ...
  scale_y_continuous(name="Sequence abundance", breaks=seq(from = 0, to = 1000, by = 100))+ #Changes y-axis title & sets the y breaks.
...
>#### <span style="color:red">CHALLENGE</span> 
If I use an updated database, will the taxonomy table different?
...
# Guidelines
For improving	the	reliability	of	ecological	results	when	analysing	eDNA	metabarcoding	data

<center>
![](Other_materials/ecoquestions.png)
</center>
From [Calderón‐Sanou et al., 2020](https://onlinelibrary.wiley.com/doi/abs/10.1111/jbi.13681).
